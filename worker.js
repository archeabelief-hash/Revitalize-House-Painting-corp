...
const BOARD_IDS = [
  ...
  ["old-intakes", "Old Intakes", "18418316511"],
  ["old-projects", "Old Projects", "18418311680"],
  ["old-clients", "Old Clients / Contacts", "18418311649"],
  ["old-sales", "Old Sales Partners", "18418322850"],
  ["old-portal", "Old Portal Access", "18418328159"],
  ["old-messages", "Old Messages / Follow-Ups", "18418324129"]
];
...
function cleanFields(item){ return (item.column_values||[]).filter(c=>c.text&&!/(password|credential|api token|secret key)/i.test(c.column?.title||"")).slice(0,14).map(c=>({title:c.column?.title||c.id,text:c.text})); }
